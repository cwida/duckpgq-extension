...
namespace duckdb {
    ...
    PropertyGraphTable *MatchFunction::FindGraphTable(const string &label, CreatePropertyGraphInfo &pg_table) {
    ...
    void MatchFunction::CheckInheritance(PropertyGraphTable *&tableref, PathElement *element,
        ...
        auto subset_compare = make_uniq<ComparisonExpression>(ExpressionType::COMPARE_EQUAL,
                                                              std::move(and_expression),
                                                              std::move(constant_expression_idx_label_comparison));
        conditions.push_back(std::move(subset_compare));
    }

    void MatchFunction::CheckEdgeTableConstraints(const string &src_reference, const string &dst_reference,
    ...
    MatchFunction::CreateMatchJoinExpression(vector<string> vertex_keys, vector<string> edge_keys,
        ...
        for (idx_t i = 0; i < vertex_keys.size(); i++) {
            ...
            conditions.push_back(
                    make_uniq<ComparisonExpression>(ExpressionType::COMPARE_EQUAL, std::move(vertex_colref),
    ...
    PathElement *MatchFunction::GetPathElement(unique_ptr<PathReference> &path_reference,
                                               vector<unique_ptr<ParsedExpression>> &conditions) {
    ...
    unique_ptr<SelectStatement> MatchFunction::GetCountTable(PropertyGraphTable *&edge_table,
    ...
    unique_ptr<JoinRef> MatchFunction::GetJoinRef(PropertyGraphTable *&edge_table, const string &edge_binding,
        ...
        second_join_ref->condition =
                make_uniq<ComparisonExpression>(ExpressionType::COMPARE_EQUAL, std::move(t_from_ref),
        ...
        first_join_ref->condition =
                make_uniq<ComparisonExpression>(ExpressionType::COMPARE_EQUAL, std::move(t_to_ref),
    ...
    unique_ptr<SubqueryRef> MatchFunction::CreateSrcDstPairsSubquery(vector<unique_ptr<ParsedExpression>> &column_list,
                                                                     const string &prev_binding,
    ...
    unique_ptr<TableRef> MatchFunction::MatchBindReplace(ClientContext &context, TableFunctionBindInput &input) {
            ...
            for (idx_t idx_j = 1; idx_j < ref->path_list[idx_i]->path_elements.size(); idx_j = idx_j + 2) {
                ...
                if (path_list->path_elements[idx_j]->path_reference_type == PGQPathReferenceType::SUBPATH) {
                    ...
                    if (subpath->upper > 1) {
                        ...
                        auto edge_rowid_colref = make_uniq<ColumnRefExpression>("rowid",
                                                                                edge_element->variable_binding);
        ...
        switch (edge_element->match_type) {
                    ...
                    case PGQMatchType::MATCH_EDGE_LEFT:
                        CheckEdgeTableConstraints(next_vertex_table->table_name, previous_vertex_table->table_name,
                                                  edge_table);
                    ...
                    case PGQMatchType::MATCH_EDGE_RIGHT:
                        CheckEdgeTableConstraints(previous_vertex_table->table_name, next_vertex_table->table_name,
                                                  edge_table);
                        conditions.push_back(CreateMatchJoinExpression(edge_table->source_pk, edge_table->source_fk,
                                                                       previous_vertex_element->variable_binding,
                                                                       edge_element->variable_binding));
                    ...
                    case PGQMatchType::MATCH_EDGE_LEFT_RIGHT: {
                        ...
                        auto additional_edge_alias =
                                edge_element->variable_binding + std::to_string(extra_alias_counter);
                        extra_alias_counter++;
