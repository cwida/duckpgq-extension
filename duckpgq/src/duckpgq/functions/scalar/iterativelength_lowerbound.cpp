...
static bool
IterativeLengthLowerBound(int64_t v_size, int64_t *v, vector<int64_t> &e,
      ...
      if (visit[i][lane]) {
        for (auto offset = v[i]; offset < v[i + 1]; offset++) {
          ...
          if (seen[n][lane] == false ||
              parents_v[i][lane].find(n) == parents_v[i][lane].end()) {
...
static void IterativeLengthLowerBoundFunction(DataChunk &args,
  ...
  while (started_searches < args.size()) {
    ...
    for (int64_t iter = 1; active && iter <= upper_bound; iter++) {
      bool stop = !IterativeLengthLowerBound(v_size, v, e, parents_v, seen,
