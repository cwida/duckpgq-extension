...
// global_sort_state.buffer_manager); 				merge_sorter.PerformInMergeRound();
// 				global_sort_state.CompleteMergeRound(true);
// 			}
// 		}
//
// 		template <typename T>
// 		static vector<T> ExtractColumn(SortedTable &table, idx_t
// col_idx) { 			vector<T> result; 			result.reserve(table.count);
//
...
//
// 			DataChunk payload;
// 			payload.Initialize(Allocator::DefaultAllocator(),
// gstate.payload_layout.GetTypes()); 			for (;;) { 				scanner.Scan(payload); 				const
// auto count = payload.size(); 				if (!count) { 					break;
// 				}
//
// 				const auto data_ptr =
// FlatVector::GetData<T>(payload.data[col_idx]); 				result.insert(result.end(),
...
// 		}
//
// 		IEJoinUnion(ClientContext &context, const PhysicalIEJoin &op,
// SortedTable &t1, const idx_t b1, SortedTable &t2, 								const idx_t b2);
//
// 		idx_t SearchL1(idx_t pos);
// 		bool NextRow();
//
// 		//! Inverted loop
// 		idx_t JoinComplexBlocks(SelectionVector &lsel, SelectionVector
// &rsel);
//
// 		//! L1
...
// idx_t IEJoinUnion::AppendKey(SortedTable &table, ExpressionExecutor
// &executor, SortedTable &marked, int64_t increment, 														 int64_t base, const idx_t
// block_idx) { 	LocalSortState local_sort_state;
// 	local_sort_state.Initialize(marked.global_sort_state,
// marked.global_sort_state.buffer_manager);
//
// 	// Reading
...
//
// 	DataChunk scanned;
// 	scanned.Initialize(Allocator::DefaultAllocator(),
// scanner.GetPayloadTypes());
//
// 	// Writing
...
// }
//
// IEJoinUnion::IEJoinUnion(ClientContext &context, const PhysicalIEJoin &op,
// SortedTable &t1, const idx_t b1, 												 SortedTable &t2, const idx_t b2) 				: n(0),
// i(0) {
// 	// input : query Q with 2 join predicates t1.X op1 t2.X' and t1.Y op2
// t2.Y', tables T, T' of sizes m and n resp.
// 	// output: a list of tuple pairs (ti , tj)
// 	// Note that T/T' are already sorted on X/X' and contain the payload
// data
// 	// We only join the two block numbers and use the sizes of the blocks as
// the counts
//
...
//
// 	// For the union algorithm, we make a unified table with the keys and
// the rids as the payload:
// 	//		X/X', Y/Y', R/R'/Li
...
// 	auto ref =
// make_uniq<BoundReferenceExpression>(order1.expression->return_type, 0);
...
//
// 	// We don't actually need the L1 column, just its sort key, which is in
// the sort blocks 	li = ExtractColumn<int64_t>(*l1, types.size() - 1);
//
...
// 	orders.clear();
// 	ref =
// make_uniq<BoundReferenceExpression>(order2.expression->return_type, 0);
...
// 	for (idx_t base = 0, block_idx = 0; block_idx < l1->BlockCount();
// ++block_idx) { 		base += AppendKey(*l1, executor, *l2, 1, base, block_idx);
// 	}
//
// 	Sort(*l2);
//
// 	// We don't actually need the L2 column, just its sort key, which is in
// the sort blocks
//
...
// 	for (const auto entry_count = bits.EntryCount(n); entry_idx <
// entry_count; ++entry_idx) { 		if (entry) { 			for (; idx_in_entry <
// bits.BITS_PER_VALUE; ++idx_in_entry, ++j) { 				if (bits.RowIsValid(entry,
// idx_in_entry)) { 					return j;
// 				}
// 			}
// 		} else {
...
// }
//
// idx_t IEJoinUnion::JoinComplexBlocks(SelectionVector &lsel, SelectionVector
// &rsel) {
...
// 			while (j < n) {
// 				auto bloom_begin = NextValid(bloom_filter, j /
// BLOOM_CHUNK_BITS, bloom_count) * BLOOM_CHUNK_BITS; 				auto bloom_end =
// MinValue<idx_t>(n, bloom_begin + BLOOM_CHUNK_BITS);
//
...
// 			}
//
// 			// Filter out tuples with the same sign (they come from
// the same table) 			const auto rrid = li[j];
// 			++j;
//
...
public:
  explicit PathFindingLocalSourceState(ClientContext &context,
    ...
    // left_executor(context), right_executor(context), left_matches(nullptr),
    // right_matches(nullptr) 	auto &allocator = Allocator::Get(context);
...
// &result, LocalSourceState &state_p) const { 	auto &state =
// state_p.Cast<IEJoinLocalSourceState>(); 	auto &ie_sink =
// sink_state->Cast<IEJoinGlobalState>(); 	auto &left_table = *ie_sink.tables[0];
...
//
// 		chunk.Reset();
// 		SliceSortedPayload(chunk, left_table.global_sort_state,
// state.left_block_index, lsel, result_count, 0); 		SliceSortedPayload(chunk,
// right_table.global_sort_state, state.right_block_index, rsel, result_count,
// 											 left_cols);
...
// 				state.left_executor.ExecuteExpression(cmp_idx,
// left);
//
// 				auto &right = state.right_keys.data[cmp_idx];
// 				state.right_executor.ExecuteExpression(cmp_idx,
// right);
//
...
// 				}
// 				tail_count = SelectJoinTail(conditions[cmp_idx +
// 2].comparison, left, right, sel, tail_count, true_sel); 				sel = true_sel;
// 			}
...
// 				left_table.found_match[state.left_base +
// lsel[sel->get_index(i)]] = true;
// 			}
// 		}
// 		if (right_table.found_match) {
// 			for (idx_t i = 0; i < result_count; i++) {
// 				right_table.found_match[state.right_base +
// rsel[sel->get_index(i)]] = true;
// 			}
// 		}
// 		result.Verify();
// 	} while (result.size() == 0);
...
public:
  idx_t MaxThreads() override {
    ...
    return 1;
    // return sink_state.tables[0]->BlockCount() *
    // sink_state.tables[1]->BlockCount();
  }
  ...
  void GetNextPair(ClientContext &client, PathFindingGlobalState &gstate,
    ...
    //
    // 	lstate.joiner = make_uniq<IEJoinUnion>(client, op, left_table, b1,
    // right_table, b2); 	return;
    // }
    ...
    //
    // 	lstate.right_matches = right_table.found_match.get() +
    // lstate.right_base; 	lstate.outer_idx = 0; 	lstate.outer_count =
    // right_table.BlockSize(r); 	return; } else { 	lstate.right_matches =
    // nullptr;
    // }
  }

  // void PairCompleted(ClientContext &client, PathFindingGlobalState &gstate,
  // PahtFindingLocalSourceState &lstate) { 	lstate.joiner.reset();
  // 	++completed;
...
PhysicalPathFinding::GetData(ExecutionContext &context, DataChunk &result,
  ...
  // if (!ie_lstate.joiner && !ie_lstate.left_matches &&
  // !ie_lstate.right_matches) { 	ie_gstate.GetNextPair(context.client, ie_sink,
  // ie_lstate);
  // }

  // Process INNER results
  // while (pf_lstate.joiner) {
  ...
  // 	chunk.Reset();
  // 	SliceSortedPayload(chunk, ie_sink.tables[0]->global_sort_state,
  // ie_lstate.left_block_index, ie_lstate.true_sel, 										 count);
  //
  ...
  // 	auto &chunk = ie_lstate.unprojected;
  // 	chunk.Reset();
  // 	SliceSortedPayload(chunk, ie_sink.tables[1]->global_sort_state,
