...
//===--------------------------------------------------------------------===//
		...
		public:
				PathFindingGlobalState(ClientContext &context, const PhysicalPathFinding &op) {
					// tables.resize(2);
...
//===--------------------------------------------------------------------===//
		...
		public:
				explicit PathFindingLocalSourceState(ClientContext &context, const PhysicalPathFinding &op)
								: op(op), true_sel(STANDARD_VECTOR_SIZE) {
// left_executor(context), right_executor(context), left_matches(nullptr), right_matches(nullptr)
		...
		public:
				...
				void GetNextPair(ClientContext &client, PathFindingGlobalState &gstate, PathFindingLocalSourceState &lstate) {
				...
				// void PairCompleted(ClientContext &client, PathFindingGlobalState &gstate, PahtFindingLocalSourceState &lstate) {
				// 	lstate.joiner.reset();
				// 	++completed;
