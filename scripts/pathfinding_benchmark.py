...
CREATE TABLE benchmark_pairs_{pair_count} AS
WITH srcs AS (
...
def run_benchmark(args):
    ...
    for repeat in range(1, args.repeats + 1):
        for mode in modes:
            ...
            sql = operator_sql(attached_db, args.pairs, args.threads, prefix) if mode == "operator" else scalar_sql(
                attached_db, args.pairs, args.threads
            )
            output, elapsed = run_duckdb(BENCH_DUCKDB, None, sql, quiet=True)
    ...
    with result_path.open("w", newline="") as handle:
        fieldnames = ["scale_factor", "mode", "threads", "repeat", "pair_count", "reachable_count", "total_len", "min_len", "max_len", "elapsed_s", "database"]
