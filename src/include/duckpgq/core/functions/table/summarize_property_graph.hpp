...
public:
	...
	static unique_ptr<GlobalTableFunctionState> SummarizePropertyGraphInit(ClientContext &context,
	                                                                       TableFunctionInitInput &input);

	static unique_ptr<SubqueryRef> CreateGroupBySubquery(const shared_ptr<PropertyGraphTable> &pg_table,
	...
	CreateDegreeStatisticsCTE(const shared_ptr<PropertyGraphTable> &pg_table, const string &degree_column,
	                          bool is_in_degree);
	static unique_ptr<ParsedExpression> GetIsolatedNodes(shared_ptr<PropertyGraphTable> &pg_table, const string &alias,
	                                                     bool is_source);
	static unique_ptr<ParsedExpression> GetDistinctCount(const shared_ptr<PropertyGraphTable> &pg_table,
	...
	static unique_ptr<TableRef> HandleSingleVertexTable(const shared_ptr<PropertyGraphTable> &vertex_table,
