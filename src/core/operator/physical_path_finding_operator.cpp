...
PathFindingGlobalSinkState::PathFindingGlobalSinkState(ClientContext &context,
  ...
  global_pairs =
      make_uniq<ColumnDataCollection>(context, op.children[0].get().GetTypes());
  global_csr_column_data =
      make_uniq<ColumnDataCollection>(context, op.children[1].get().GetTypes());
