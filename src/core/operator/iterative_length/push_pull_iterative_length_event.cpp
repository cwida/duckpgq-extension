...
static mutex push_pull_bfs_phase_timing_lock;
...
static void AppendBFSPhaseTiming(const PushPullIterativeLengthState &state, double time_ms) {
	auto file_name = state.benchmark_output_prefix + "_phase_timing.csv";
	lock_guard<mutex> lock(push_pull_bfs_phase_timing_lock);
...
PushPullIterativeLengthEvent::PushPullIterativeLengthEvent(shared_ptr<PushPullIterativeLengthState> gbfs_state_p,
                                                           Pipeline &pipeline_p, const PhysicalPathFinding &op_p)
    : BasePipelineEvent(pipeline_p), gbfs_state(std::move(gbfs_state_p)), op(op_p) {
}

void PushPullIterativeLengthEvent::Schedule() {
...
void PushPullIterativeLengthEvent::FinishEvent() {
	...
	auto file_name = gbfs_state->benchmark_output_prefix + "_pushpull_explore_timing_" + gbfs_state->benchmark_run_id +
	                 "_threads_" + std::to_string(gbfs_state->num_threads) + "_" + heavy_partition_fraction + "_" +
