...
IterativeLengthState::IterativeLengthState(const shared_ptr<DataChunk> &pairs_, std::vector<shared_ptr<LocalCSR>> &local_csrs_,
  std::vector<std::pair<idx_t, idx_t>> &partition_ranges_, idx_t num_threads_, ClientContext &context_, int64_t vsize_)
    : BFSState(pairs_, local_csrs_, partition_ranges_, num_threads_, "iterativelength", context_, vsize_) {
...
void IterativeLengthState::WriteTimingResults(const std::string &filename) {
  ...
  if (file.is_open()) {
    file << "ThreadID,CoreID,Time(ms),ThreadCount,vsize,esize,numPartitions\n";
    for (const auto &entry : timing_data) {
      file << std::get<0>(entry) << "," << std::get<1>(entry) << "," << std::get<2>(entry) << "," << std::get<3>(entry) << "," << std::get<4>(entry) << "," << std::get<5>(entry) << "," << std::get<6>(entry) << "\n";
    }
    file.close();
  }
}
