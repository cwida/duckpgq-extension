...
static bool IsCSRIdProjection(const LogicalProjection &projection) {
  for (const auto &expr : projection.expressions) {
    ...
    if (expr->GetExpressionType() == ExpressionType::OPERATOR_CAST && expr->Cast<BoundCastExpression>().GetName() == "csr_id") {
      return true;
    }
  }
  return false;
...
unique_ptr<Expression> CreateReplacementExpression(const Identifier &alias, const string &functionName, TableIndex tableIndex, idx_t position) {
  if (functionName == "iterativelengthoperator") {
    return make_uniq<BoundColumnRefExpression>(alias, LogicalType::BIGINT, ColumnBinding(tableIndex, ProjectionIndex(position)));
  }
  if (functionName == "shortestpathoperator") {
    return make_uniq<BoundColumnRefExpression>(alias, LogicalType::LIST(LogicalType::BIGINT), ColumnBinding(tableIndex, ProjectionIndex(position)));
  }
  return nullptr;
}
