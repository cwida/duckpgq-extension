...
unique_ptr<FunctionData> DescribePropertyGraphFunction::DescribePropertyGraphBind(ClientContext &context,
	...
	auto statement = dynamic_cast<SelectStatement *>(duckpgq_parse_data->statement.get());
	auto select_node = dynamic_cast<SelectNode *>(statement->node.get());
	auto show_ref = dynamic_cast<ShowRef *>(select_node->from_table.get());

	auto pg_table = duckpgq_state->registered_property_graphs.find(show_ref->table_name);
...
DescribePropertyGraphFunction::DescribePropertyGraphInit(ClientContext &context, TableFunctionInitInput &input) {
	return make_uniq<DescribePropertyGraphGlobalData>();
}

void DescribePropertyGraphFunction::DescribePropertyGraphFunc(ClientContext &context, TableFunctionInput &data_p,
                                                              DataChunk &output) {
