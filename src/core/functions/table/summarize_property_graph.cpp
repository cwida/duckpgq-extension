...
GetTableNameConstantExpression(const string &table_name, const string &alias) {
...
unique_ptr<ParsedExpression>
GetFunctionExpression(const string &aggregate_function, const string &alias,
                      bool is_in_degree, const Value &value = nullptr) {
  vector<unique_ptr<ParsedExpression>> max_children;
  max_children.push_back(make_uniq<ColumnRefExpression>(
      is_in_degree ? "in_degree" : "out_degree"));
  if (!value.IsNull()) {
    max_children.push_back(make_uniq<ConstantExpression>(value));
  }
  auto agg_function = make_uniq<FunctionExpression>(aggregate_function,
                                                    std::move(max_children));
  agg_function->alias = alias + (is_in_degree ? "_in_degree" : "_out_degree");
  return agg_function;
}

unique_ptr<ParsedExpression>
GetConstantNullExpressionWithAlias(const string &alias) {
...
unique_ptr<ParsedExpression>
IsVertexTableConstantExpression(bool is_vertex_table, const string &alias) {
...
unique_ptr<ParsedExpression> GetTableCount(const string &alias) {
  ...
  auto aggregate_function =
      make_uniq<FunctionExpression>("count_star", std::move(children));
  aggregate_function->alias = alias;
  return std::move(aggregate_function);
}

unique_ptr<ParsedExpression> SummarizePropertyGraphFunction::GetDistinctCount(
    const shared_ptr<PropertyGraphTable> &pg_table, const string &alias,
    bool is_source) {
  auto result = make_uniq<SubqueryExpression>();
  result->subquery_type = SubqueryType::SCALAR;
  auto select_statement = make_uniq<SelectStatement>();
  auto select_node = make_uniq<SelectNode>();
  select_node->from_table = make_uniq<BaseTableRef>(TableDescription(
      pg_table->catalog_name, pg_table->schema_name, pg_table->table_name));
  vector<unique_ptr<ParsedExpression>> count_children;
  auto column_to_count =
      is_source ? pg_table->source_fk[0] : pg_table->destination_fk[0];
  count_children.push_back(
      make_uniq<ColumnRefExpression>(column_to_count, pg_table->table_name));
  auto count_expression =
...
unique_ptr<ParsedExpression> SummarizePropertyGraphFunction::GetIsolatedNodes(
    shared_ptr<PropertyGraphTable> &pg_table, const string &alias,
    bool is_source) {
  auto result = make_uniq<SubqueryExpression>();
  result->subquery_type = SubqueryType::SCALAR;
  auto select_statement = make_uniq<SelectStatement>();
  ...
  string table_reference =
      is_source ? pg_table->source_reference : pg_table->destination_reference;
  string table_schema =
      is_source ? pg_table->source_schema : pg_table->destination_schema;
  string table_catalog =
      is_source ? pg_table->source_catalog : pg_table->destination_catalog;
  string pk_reference =
      is_source ? pg_table->source_pk[0] : pg_table->destination_pk[0];
  string fk_reference =
      is_source ? pg_table->source_fk[0] : pg_table->destination_fk[0];
  count_children.push_back(
  ...
  auto source_table_ref = make_uniq<BaseTableRef>(
      TableDescription(table_catalog, table_schema, table_reference));
  auto edge_table_ref = make_uniq<BaseTableRef>(TableDescription(
      pg_table->catalog_name, pg_table->schema_name, pg_table->table_name));
  ...
  operator_children.push_back(
      make_uniq<ColumnRefExpression>(fk_reference, pg_table->table_name));
  auto operator_expression = make_uniq<OperatorExpression>(
      ExpressionType::OPERATOR_IS_NULL, std::move(operator_children));
...
unique_ptr<SubqueryRef> SummarizePropertyGraphFunction::CreateGroupBySubquery(
    const shared_ptr<PropertyGraphTable> &pg_table, bool is_in_degree,
    const string &degree_column) {
  auto select_node = make_uniq<SelectNode>();
  select_node->select_list.push_back(
      make_uniq<ColumnRefExpression>(degree_column));
  vector<unique_ptr<ParsedExpression>> children;
  auto count_star_expression =
      make_uniq<FunctionExpression>("count_star", std::move(children));
  ...
  grouping_node.group_expressions.push_back(
      make_uniq<ColumnRefExpression>(degree_column));
...
SummarizePropertyGraphFunction::CreateDegreeStatisticsCTE(
    const shared_ptr<PropertyGraphTable> &pg_table, const string &degree_column,
    bool is_in_degree) {
  auto cte_info = make_uniq<CommonTableExpressionInfo>();
  auto select_statement = make_uniq<SelectStatement>();
  auto select_node = make_uniq<SelectNode>();
  select_node->select_list.push_back(
      GetFunctionExpression("avg", "avg", is_in_degree));
  select_node->select_list.push_back(
      GetFunctionExpression("min", "min", is_in_degree));
  select_node->select_list.push_back(
      GetFunctionExpression("max", "max", is_in_degree));
  select_node->select_list.push_back(GetFunctionExpression(
      "approx_quantile", "q25", is_in_degree, Value::FLOAT(0.25)));
  select_node->select_list.push_back(GetFunctionExpression(
      "approx_quantile", "q50", is_in_degree, Value::FLOAT(0.5)));
  select_node->select_list.push_back(GetFunctionExpression(
      "approx_quantile", "q75", is_in_degree, Value::FLOAT(0.75)));
  select_node->from_table =
      CreateGroupBySubquery(pg_table, is_in_degree, degree_column);
  select_statement->node = std::move(select_node);
...
unique_ptr<ParsedExpression>
SummarizePropertyGraphFunction::GetDegreeStatistics(
    const string &aggregate_function, bool is_in_degree) {
  auto result = make_uniq<SubqueryExpression>();
  result->subquery_type = SubqueryType::SCALAR;
  auto select_statement = make_uniq<SelectStatement>();
  auto select_node = make_uniq<SelectNode>();
  select_node->select_list.push_back(make_uniq<ColumnRefExpression>(
      aggregate_function + "_" + (is_in_degree ? "in_degree" : "out_degree")));
...
unique_ptr<CommonTableExpressionInfo>
SummarizePropertyGraphFunction::CreateVertexTableCTE(
    const shared_ptr<PropertyGraphTable> &vertex_table) {
  auto cte_info = make_uniq<CommonTableExpressionInfo>();
  auto select_statement = make_uniq<SelectStatement>();
  auto select_node = make_uniq<SelectNode>();
  select_node->select_list.push_back(
      GetTableNameConstantExpression(vertex_table->table_name, "table_name"));
  select_node->select_list.push_back(
      IsVertexTableConstantExpression(true, "is_vertex_table"));
  select_node->select_list.push_back(GetConstantNullExpressionWithAlias(
  ...
  select_node->select_list.push_back(GetConstantNullExpressionWithAlias(
      "edge_count")); // edge_count (NULL for vertex tables)
  select_node->select_list.push_back(
      GetConstantNullExpressionWithAlias("unique_source_count"));
  select_node->select_list.push_back(
      GetConstantNullExpressionWithAlias("unique_destination_count"));
  select_node->select_list.push_back(
      GetConstantNullExpressionWithAlias("isolated_sources"));
  select_node->select_list.push_back(
      GetConstantNullExpressionWithAlias("isolated_destinations"));
  select_node->select_list.push_back(
      GetConstantNullExpressionWithAlias("avg_in_degree"));
  select_node->select_list.push_back(
      GetConstantNullExpressionWithAlias("min_in_degree"));
  select_node->select_list.push_back(
      GetConstantNullExpressionWithAlias("max_in_degree"));
  select_node->select_list.push_back(
      GetConstantNullExpressionWithAlias("q25_in_degree"));
  select_node->select_list.push_back(
      GetConstantNullExpressionWithAlias("q50_in_degree"));
  select_node->select_list.push_back(
      GetConstantNullExpressionWithAlias("q75_in_degree"));
  select_node->select_list.push_back(
      GetConstantNullExpressionWithAlias("avg_out_degree"));
  select_node->select_list.push_back(
      GetConstantNullExpressionWithAlias("min_out_degree"));
  select_node->select_list.push_back(
      GetConstantNullExpressionWithAlias("max_out_degree"));
  select_node->select_list.push_back(
      GetConstantNullExpressionWithAlias("q25_out_degree"));
  select_node->select_list.push_back(
      GetConstantNullExpressionWithAlias("q50_out_degree"));
  select_node->select_list.push_back(
      GetConstantNullExpressionWithAlias("q75_out_degree"));
  select_node->from_table = make_uniq<BaseTableRef>(
      TableDescription(vertex_table->catalog_name, vertex_table->schema_name,
...
unique_ptr<CommonTableExpressionInfo>
SummarizePropertyGraphFunction::CreateEdgeTableCTE(
    shared_ptr<PropertyGraphTable> &edge_table) {
  auto cte_info = make_uniq<CommonTableExpressionInfo>();
  auto select_statement = make_uniq<SelectStatement>();
  auto select_node = make_uniq<SelectNode>();
  select_node->cte_map.map.insert(
      "in_degrees", CreateDegreeStatisticsCTE(
                        edge_table, edge_table->destination_fk[0], true));
  select_node->cte_map.map.insert(
      "out_degrees",
      CreateDegreeStatisticsCTE(edge_table, edge_table->source_fk[0], false));

  select_node->select_list.push_back(
      GetTableNameConstantExpression(edge_table->table_name, "table_name"));
  select_node->select_list.push_back(
      IsVertexTableConstantExpression(false, "is_vertex_table"));
  select_node->select_list.push_back(GetTableNameConstantExpression(
  ...
  select_node->select_list.push_back(GetTableNameConstantExpression(
      ...
      "destination_table")); // destination table name (NULL for vertex tables)
  select_node->select_list.push_back(
      GetConstantNullExpressionWithAlias("vertex_count"));
  select_node->select_list.push_back(GetTableCount("edge_count"));

  select_node->select_list.push_back(
      GetDistinctCount(edge_table, "unique_source_count", true));
  select_node->select_list.push_back(
      GetDistinctCount(edge_table, "unique_destination_count", false));

  select_node->select_list.push_back(
      GetIsolatedNodes(edge_table, "isolated_sources", true));
  select_node->select_list.push_back(
      GetIsolatedNodes(edge_table, "isolated_destinations", false));
  ...
  select_node->from_table = make_uniq<BaseTableRef>(
      TableDescription(edge_table->catalog_name, edge_table->schema_name,
...
unique_ptr<TableRef> SummarizePropertyGraphFunction::HandleSingleVertexTable(
    const shared_ptr<PropertyGraphTable> &vertex_table,
    const string &stat_table_alias) {
  auto select_node = make_uniq<SelectNode>();
  select_node->cte_map.map.insert(stat_table_alias,
                                  CreateVertexTableCTE(vertex_table));
...
void AddToUnionNode(unique_ptr<SetOperationNode> &final_union_node,
                    unique_ptr<SelectNode> &inner_select_node) {
...
CreateInnerSelectStatNode(const string &stat_table_alias) {
...
unique_ptr<TableRef>
SummarizePropertyGraphFunction::SummarizePropertyGraphBindReplace(
    ClientContext &context, TableFunctionBindInput &bind_input) {
  ...
  for (auto &vertex_table : pg_info->vertex_tables) {
    ...
    inner_select_node->cte_map.map.insert(stat_table_alias,
                                          CreateVertexTableCTE(vertex_table));
    AddToUnionNode(final_union_node, inner_select_node);
  }
  for (auto &edge_table : pg_info->edge_tables) {
    string stat_table_alias = edge_table->source_reference + "_" +
                              edge_table->table_name + "_" +
                              edge_table->destination_reference + "_stats";
    auto inner_select_node = CreateInnerSelectStatNode(stat_table_alias);
    inner_select_node->cte_map.map.insert(stat_table_alias,
                                          CreateEdgeTableCTE(edge_table));
    AddToUnionNode(final_union_node, inner_select_node);
  }
